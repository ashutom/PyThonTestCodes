...
#from data.dle_utils import dboxes300_coco, build_ssd300_coder
...
def train300_mlperf_coco(args):
    ...
    print('SSD300 model {}'.format(ssd300))
    tensor_checker.init(ssd300)
    ...
    for epoch in range(args.epochs):
        ...
        #Uncomment this when using DALI
        #for i, (img, bbox, label) in enumerate(train_loader):
        #debug_overflow = DebugUnderflowOverflow(ssd300)
        #print(f"Ashutosh: DebugUnderflowOverflow  Called " )
        #with torch.autograd.detect_anomaly():
        #Enable on ROCm
        for i, (img, _, _, bbox, label) in enumerate(train_loader):
            ...
            label = label.cuda()
            bbox = Variable(trans_bbox, requires_grad=False)
            ...
            torch.cuda.synchronize()
            tensor_checker.check_grads()
            tensor_checker.reset()
            torch.cuda.synchronize()
            ...
            iter_num += 1
        #Enable on ROCm; Uncomment this when using DALI
        #train_loader.reset()
